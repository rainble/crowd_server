...
import fudan.mcd.service.SearchTemplateService;
import fudan.mcd.servlet.SearchTemplateServlet.ResponseBO.TemplateBO;
...
@WebServlet("/SearchTemplateServlet")
public class SearchTemplateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Log LOG = LogFactory.getLog(SearchTemplateServlet.class);

	public SearchTemplateServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			templateName = requestBO.templateName;
		}
		catch (Exception e) {
			...
			LOG.info(String.format("Receive search template request [ parameter parse failed ] at [ %s ].", ServletUtils.getTime()));
			return;
		}

		// Do business operation
		SearchTemplateService service = new SearchTemplateService(getServletContext());
		List<SearchTemplateService.ResultVO> rvoList = service.searchTemplate(userId, templateName);

		// Generate response data
		ServletResponseData responseData = new ServletResponseData();
		ResponseBO responseBO = new ResponseBO();
		List<TemplateBO> templates = new ArrayList<TemplateBO>();
		for (SearchTemplateService.ResultVO rvo : rvoList) {
