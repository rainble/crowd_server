...
import fudan.mcd.service.QueryTaskInfoService;
import fudan.mcd.servlet.QueryTaskInfoServlet.ResponseBO.InputBO;
import fudan.mcd.servlet.QueryTaskInfoServlet.ResponseBO.LocationBO;
import fudan.mcd.servlet.QueryTaskInfoServlet.ResponseBO.OutputBO;
import fudan.mcd.servlet.QueryTaskInfoServlet.ResponseBO.StageBO;
...
//		int userId = 1;
		...
		try {
			...
			LOG.info(String.format("Receive query task info request [ parameter parse failed ] at [ %s ].", ServletUtils.getTime()));
			return;
		}

		// Do business operation
		QueryTaskInfoService service = new QueryTaskInfoService(getServletContext());
		QueryTaskInfoService.ResultVO rvo = service.queryTaskInfo(taskId,userId);
