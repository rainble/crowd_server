...
import fudan.mcd.service.GetUserInfoService;
...
public class GetUserInfoServlet extends HttpServlet {
	...
	private static final Log LOG = LogFactory.getLog(GetUserInfoServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//LOG.info(String.format("receive getUserInfo request [ userId = %d ].", requestBO.getUserId()));
		}
		catch (Exception e) {
			...
			LOG.info(String.format("Receive get user info request [ parameter parse failed ] at [ %s ].", ServletUtils.getTime()));
			return;
		}

		// Do business operation
		GetUserInfoService service = new GetUserInfoService(getServletContext());
		UserVO uvo = service.getUserInfo(userId);
		...
		if(uvo != null){
			...
			responseBO.setUserTag(uvo.getTag());
			responseBO.setLoginFlag(uvo.getLoginFlag());
